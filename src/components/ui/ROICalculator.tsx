...
import { LoadingSpinner, ROICalculatorSkeleton } from './LoadingSpinner'
import { ROICalculatorErrorBoundary } from './ErrorBoundary'
...
const PROPERTY_TYPES = [
  { id: 'office_building' as CommercialPropertyType, name: 'Office Building', icon: '🏢' },
  { id: 'retail_space' as CommercialPropertyType, name: 'Retail Space', icon: '🏪' },
  { id: 'warehouse' as CommercialPropertyType, name: 'Warehouse', icon: '🏭' },
  { id: 'hospitality' as CommercialPropertyType, name: 'Hotel/Hospitality', icon: '🏨' },
  { id: 'healthcare' as CommercialPropertyType, name: 'Healthcare', icon: '🏥' },
  { id: 'education' as CommercialPropertyType, name: 'Education', icon: '🏫' }
] as const
...
// Memoized compact component
const CompactROIDisplay = memo(({ calculations, className }: { 
  calculations: any,
  className: string 
}) => {
...
CompactROIDisplay.displayName = 'CompactROIDisplay'
...
function ROICalculatorInner({ 
  ...
  const { onMount, measureOperation } = usePerformanceMonitoring('ROICalculator')
  ...
  const handlePropertyTypeChange = useCallback((type: CommercialPropertyType) => {
    measureOperation('propertyTypeChange', () => {
      setPropertyType(type)
    })
  }, [measureOperation])
  
  const handleFloorAreaChange = useCallback((value: string) => {
    setFloorArea(value)
  }, [])
  
  const handleEPCChange = useCallback((rating: EPCRating) => {
    setCurrentEPC(rating)
  }, [])
  
  const handleBudgetChange = useCallback((budgetLevel: 'low' | 'medium' | 'high' | 'unlimited') => {
    setBudget(budgetLevel)
  }, [])

  // Optimized calculations with performance monitoring
  ...
  if (isCalculating) {
    return (
      <Card className={`p-6 ${className}`}>
        <LoadingSpinner size="lg" text="Calculating ROI projections..." />
      </Card>
    )
  }
  ...
  if (compact) {
    return <CompactROIDisplay calculations={calculations} className={className} />
  }
      ...
      <Card className="p-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            <select
              value={propertyType}
              onChange={(e) => handlePropertyTypeChange(e.target.value as CommercialPropertyType)}
          ...
          <div>
            ...
            <select
              value={currentEPC}
              onChange={(e) => handleEPCChange(e.target.value as EPCRating)}
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
          ...
          <div>
            ...
            <select
              value={budget}
              onChange={(e) => handleBudgetChange(e.target.value as any)}
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
...
export const ROICalculator = memo(function ROICalculator(props: ROICalculatorProps) {
  return (
    <ROICalculatorErrorBoundary>
